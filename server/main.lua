...
function UpdateQueuePositions()
    ...
    for i = #Queue, 1, -1 do
        ...
        if not GetPlayerName(player.source) then
            ...
            local baseText = Config.QueueMessage
            if position == 1 then
                baseText = Config.NextPersonMessage
            end
            ...
            local card = {
                ...
                body = {
                    {
                        type = "TextBlock",
                        text = string.format("🎮 " ..Config.ServerName..  " Queue | Players: %d/%d", playerCount, Config.MaxSlots),
                        weight = "Bolder",
                        size = "Large",
                    ...
                    {
                        type = "TextBlock",
                        text = string.format(baseText, position, #Queue, minutes, seconds),
                        wrap = true,
                        horizontalAlignment = "Center"
                    }
                }
            ...
            if Config.ShowAmountOfQueuePlayers and Config.ShowAmountOfQueuePlayers > 0 then
                ...
                for j = 1, math.min(Config.ShowAmountOfQueuePlayers, #Queue) do
                    local qPlayer = Queue[j]
                    table.insert(card.body, {
                        type = "TextBlock",
                        text = string.format("Queue Power: %d | %s", qPlayer.rank.power, qPlayer.name),
                        wrap = true,
                        horizontalAlignment = "Center"
                    })
                end
            ...
            table.insert(card.body, {
                type = "TextBlock",
                text = "Hate waiting in the queue? Grab a supporter role from our store!",
                size = "Small",
                wrap = true,
                horizontalAlignment = "Center",
                separator = true,
            ...
            player.deferrals.presentCard(card, function() end)
...
AddEventHandler('playerDropped', function(reason)
    ...
    if discordID then
        GracePlayers[discordID] = GracePlayers[discordID] or {}
        GracePlayers[discordID].expires = GetGameTimer() + Config.GracePeriod
        GracePlayers[discordID].name = name
        print(string.format("^1[Queue] Player Disconnected: %s | Grace Period Started | Removed from Queue: %s^7", name, tostring(removed)))
    end    
...
CreateThread(function()
    while true do
        Wait(0)
        for discordID, data in pairs(GracePlayers) do
            if GetGameTimer() >= data.expires then
                local expiredName = data.name or "Unknown"
                GracePlayers[discordID] = nil
                print(string.format("^1[Queue] Grace Period Expired For: %s^7", expiredName))
            end
        end
    end
end)
